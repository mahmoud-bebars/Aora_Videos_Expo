...
export const getUserBookmarks = async (userId) => {
  try {
    ...
    return currentUser.documents[0].videoLikes;
  } catch (error) {
    console.log(error);
  }
};
...
export const savePost = async (video, userId) => {
  try {
    const updatedPost = await databases.updateDocument(
      ...
      video.$id,
      {
        videoLikes: [...video.videoLikes, userId],
      }
    );
    return updatedPost;
  } catch (error) {
...
export const unSavePost = async (video, userId) => {
  try {
    const filteredLikes = video.likedByUsers.filter((like) => like !== userId);
    console.log(video);
    const updatedPost = await databases.updateDocument(
      databaseId,
      videoCollactionId,
